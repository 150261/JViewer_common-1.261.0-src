...
public class PacketMaster
{
    ...
    /**
     * Connect to the previously specified remote host and port
     * @throws IOException on network failures
     */
    public void connectVmedianonssl() throws IOException
    {
    	packetSock = new Socket();
        try
        {
           	packetSock.connect(new InetSocketAddress( host, port ));
        }
        catch( UnresolvedAddressException e )
        {
            throw new IOException( LocaleStrings.getString("8_1_PACKETMAST") + host );
        }
        setSock(packetSock);
    }
    ...
    public void connectVmediaSSl() throws IOException
    {
        ...
        try
        {
        	packetSockssl = (SSLSocket) sf.createSocket(host, port);
        	packetSockssl.startHandshake();
            setSock(packetSockssl);
        }
        catch( UnresolvedAddressException e )
        {
        	throw new IOException( LocaleStrings.getString("8_1_PACKETMAST") + host );
