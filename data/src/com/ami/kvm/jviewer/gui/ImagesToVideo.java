...
public class ImagesToVideo extends TimerTask{
	...
	public ImagesToVideo(String movFile, String imagePath, long numOfFrames,long duration){
        ...
        try{
            out = new AVIOutputStream(new File(movFile), AVIOutputStream.VideoFormat.JPG, singleVideo);
	...
	public void run(){
		try{
			for (count = 0; count < numOfFrames; count++) {  
	        	if(! singleVideo && out.streamClosed)
	        		multipleVideos();
			...
			if(JViewerApp.getInstance().getM_frame().videoStopError != null)
				...
				if(JViewer.isdownloadapp()){
					...
					JOptionPane.showMessageDialog (
							mframe,
							LocaleStrings.getString("L_1_ITOV")+movFile+LocaleStrings.getString("L_2_ITOV"),
							LocaleStrings.getString("L_3_ITOV"),
			...
			InfoDialog.showDialog(JViewer.getMainFrame(),
					...
					(int)VideoRecord.Avg_fps+LocaleStrings.getString("L_5_ITOV"),
					LocaleStrings.getString("L_3_ITOV"),
	...
	private void multipleVideos(){
		try{			
				count--;
	    		out.streamClosed = false;
	    		out = new AVIOutputStream(new File(movFile+"_"+i+".avi"), AVIOutputStream.VideoFormat.JPG, singleVideo);	    		
	    		out.setFrameRate(numOfFrames);	    		
	    		out.setTimeScale(duration);
