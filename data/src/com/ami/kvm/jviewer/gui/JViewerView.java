...
public class JViewerView extends JComponent {
    ...
    private BufferedImage invisCursor = null;
	...
	public void ShowCursor(boolean show)
	{
			if(m_USBMouseMode == USBMouseRep.RELATIVE_MOUSE_MODE)
			{
				...
				if(invisCursor == null){
					invisCursor = new BufferedImage( 32, 32, BufferedImage.TYPE_INT_ARGB_PRE );
					Cursor invisibleCursor = Toolkit.getDefaultToolkit().createCustomCursor( invisCursor,
				...
				if(show){
					...
					BufferedImage invisCursor = new BufferedImage( 32, 32, BufferedImage.TYPE_INT_ARGB_PRE );
					Cursor invisibleCursor = Toolkit.getDefaultToolkit().createCustomCursor( invisCursor,
							new Point( 0, 0 ), "Invisible Cursor" );
					setCursor( invisibleCursor );
				}
	...
	public void SetUSBMouseMode(byte mode) {
		if(initMouseMode){
			...
			InfoDialog.showDialog(JViewer.getMainFrame(), LocaleStrings.getString("E_6_JVIEW"), 5000);
		}
