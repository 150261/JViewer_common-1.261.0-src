...
public abstract class JVMenu {
	...
	public void changeMenuItemLanguage() {
		...
		while (itr.hasNext())
		{
			try{
				...
				//To Change Language name's language eg : EN - English
				if(menu.getText().charAt(3) == '-'){
					//Retrive Language code
					stringStart = menu.getText().substring(0, 2);

					Locale locale = new Locale(stringStart.toLowerCase());
					stringEnd = locale.getDisplayLanguage(new Locale(JViewer.getLanguage()));
					key = stringStart.concat(" ").concat("-").concat(" ").concat(stringEnd);
					if(key != null){
						menu.setText(key);
					}
				}
	...
	*/
		...
		while (itr.hasNext())
		{
			try{
				...
				if(value.toString().startsWith(LocaleStrings.getPreviousLocaleString("F_119_JVM"))){
					//Retrive Language code
					stringStart = value.toString().substring(value.toString().indexOf(':')+2,value.toString().indexOf("-")-1);

					Locale locale = new Locale(stringStart.toLowerCase());
					stringEnd = locale.getDisplayLanguage(new Locale(JViewer.getLanguage()));
					key = LocaleStrings.getString("F_119_JVM")+" "+stringStart+" - "+stringEnd;
					...
					if(key !=null){
						try{
							...
							Debug.out.println(e);
							//get soc menu text here as key is not matched with common menu key exception will be thrown
							text = LocaleStrings.getSOCString(key);
						}
						m_menustatusbar_text.put(keyvalue.toString(), text);
					}
    ...
    private  class JVMenuListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
			...
			else if (cmdStr.equals(VIDEO_CAPTURE_SCREEN))
			{
				...
				RCApp.OnGUILanguageChange(menu.getText().substring(0, 2));
			}
			...
			else if (cmdStr.equals(AUTOMAIC_LANGUAGE)) {
				JCheckBoxMenuItem evtSrc = (JCheckBoxMenuItem) e.getSource();
            ...
            else if(cmdStr.equals(ACTUAL_SIZE)||
            		cmdStr.equals(FIT_TO_CLIENT_RES)||
            		cmdStr.equals(FIT_TO_HOST_RES)){
	...
	private JMenu createMenu(String menuName, String actionCommand, char mnemonic, String statusText){
	 ...
	 * return menu
	 */
	private JMenuItem createMenuItem( String menuName, char mnemonic, int keyCode, int modifiers, 
...
//		 construct bandwidth submenu
		...
		String[] newItems = StandAloneConnectionDialog.getSupportedLocales();

		for(int i =0;i<newItems.length ; i++){
			language = newItems[i].substring(0,2);
	...
	protected JMenu constructKeyboardLayoutMenu()
	{
		...
		menuItem = createCheckBoxMenuItem(LocaleStrings.getString("F_72_JVM"), ' ', AUTOMAIC_LANGUAGE, LocaleStrings.getString("F_73_JVM"));
		menu.add(menuItem);
		...
		menuItem = createRadioButtonMenu(LocaleStrings.getString("F_80_JVM"), PKBRD_LANGUAGE_FRENCH, LocaleStrings.getString("F_75_JVM")+LocaleStrings.getString("F_80_JVM")+LocaleStrings.getString("F_116_JVM"));
		group.add(menuItem);
		subMenu.add(menuItem);
...
//		 construct Hebrew menu item
		menuItem = createRadioButtonMenu(LocaleStrings.getString("F_91_JVM"), SKBRD_LANGUAGE_DUTCH, LocaleStrings.getString("F_75_JVM")+LocaleStrings.getString("F_91_JVM")+LocaleStrings.getString("F_76_JVM"));
		group.add(menuItem);
		subMenu.add(menuItem);
...
//		 construct Turkish - Q menu item
    ...
    protected JMenu constructVideoRecordMenu() {
        ...
        menuItem = createMenuItem(LocaleStrings.getString("F_98_JVM"), ' ', VIDEO_RECORD_START, LocaleStrings.getString("F_99_JVM"), false);
        menu.add(menuItem);
