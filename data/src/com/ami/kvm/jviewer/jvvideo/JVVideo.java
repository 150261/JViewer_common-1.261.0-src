...
public class JVVideo {
	...
	private Timer m_frTmr; // frame rate update timer
	...
	public JVVideo() {
		...
		if(!JViewer.isWebPreviewer() && !JViewer.isBSODViewer()) {
			...
			m_frameRate = 0;
			m_frTmr = new Timer();
			m_frTmr.schedule(new FrameRateTask(), 0, 1000);
		}
	}
	...
	public void setFullScreenMode(){
		...
		if (sd.height == m_frameHdr.getresY() && sd.width == m_frameHdr.getresX()){
			...
			JViewerApp.getInstance().getM_wndFrame().toolbar.fullScreenBtn.setToolTipText(LocaleStrings.getString("Q_6_JVT"));
		}
		else {
			...
			JViewerApp.getInstance().getM_wndFrame().toolbar.fullScreenBtn.setToolTipText(LocaleStrings.getString("D_29_JVAPP"));
		}
	}
	/**
